...
export const sendEmailRegister = async (userInfo) => {
  try {
    let axiosResponse = await axios.post(`${API_LINK}/api/users/ecreate`, userInfo)
...
export const sendSMSRegister = async (userInfo) => {
  try {
    let axiosResponse = await axios.post(`${API_LINK}/api/users/screate`, userInfo)
...
export const sendVerifyCheck = async (email, verifyCode) => { // Basically, if there is no error then the verify code was correct!
  try {
    let axiosResponse = await axios.get(`${API_LINK}/api/users/verification?email=${email}&smscode=${verifyCode}`)
    let serverResponse = axiosResponse.data 
    return true;
