...
import { update_form_values } from '../redux/actions/signupactions.js';
...
const Cform = (props) => {
	...
	const buttonhandler = (events) => {
		...
		let isCountryCodeInPhonenumber =
			phonenumber.includes('+') && phonenumber.length > 11;
		let countrycode = isCountryCodeInPhonenumber
			? phonenumber.slice(0, phonenumber.length - 10)
			: 'N/A'; // Slicing all of first digits before the last 10
					...
					<div className={styles['left-container']}>
						...
						<form className={styles['field-container']}>
							...
							<label htmlFor="gender">
								PRONOUNS<span className={styles['required']}>*</span>
							</label>
							<br />
							...
							<button id="submit-button" onClick={buttonhandler}>
								SAVE & CONTINUE
							</button>
						</form>
					</div>
