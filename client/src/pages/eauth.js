...
const EAuth = (props) => {
	...
	const handleClick = async (evt) => {
		...
		if (wasValidCode) history.push('/Profile');
		else setErrorText('SMS Code was invalid...');
	};
		...
		<div>
			<div className="container">
				...
				<form>
					...
					/>
					<button className={styles['authsubmit']} onClick={handleClick}>
						<span className={styles['button-text']}>Verify Now</span>
					</button>
					<span className={styles['SMSErrorText']}>
						{!!errorText && errorText}
					</span>
				</form>
			</div>
