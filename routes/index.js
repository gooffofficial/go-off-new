...
router.get('/feed', auth.required, (req, res, next) => {
    ...
    seq.query(
        'SELECT article FROM test_server1.SavedArticles S, test_server1.Followers Fol WHERE Fol.follower = ' +
            id +
            ' AND Fol.followed=S.userId ORDER BY S.createdAt DESC LIMIT 4'
    ).then(async (articles) => {
        var arts = [];
        ...
        seq.query(
            'SELECT ConvoId FROM test_server1.Convo_members C, test_server1.Followers Fol WHERE Fol.follower = ' +
                id +
                ' AND Fol.followed=C.UserId ORDER BY C.createdAt DESC LIMIT 4'
        )
...
=======
            ...
            for (const convo of convos){
>>>>>>> 0bc4620d02f9e2d47092c5317415e0d0c23c8e23
                    ...
                    for (let i = 0; i < dms.length; i++) {
                        for (let k = 0; k < dms[i].users.length; k++) {
                            if (dms[i].users[k] != id) {
                                lastMessages.push([
                                    dms[i].users[k],
                                    dms[i].messages[dms[i].messages.length - 1]
                                        .message,
                                ]);
                            }
                        }
